...
class OutputManager:
    ...
    def save_image(
        ...
        for profile in self.profiles:
            try:
                ...
                if (image.width != profile.width or 
                ...
                logger.error(
                    f"Failed to save image to {profile.name}: {e}",
                    exc_info=True
                )
    ...
    def _cleanup_archives(self, profile: OutputProfile, base_name: str) -> None:
        ...
        archives = sorted(
            profile.archive_dir.glob(pattern),
    ...
    def cleanup_old_files(self, days: int) -> None:
        ...
        for profile in self.profiles:
            ...
            if deleted_count > 0:
                logger.info(
                    f"Cleaned up {deleted_count} old file(s) from {profile.name}"
                )
